...
with col3:
    st.metric("Average Price", "$" + str(round(filtered_df["Sale_Median_Now"].mean())))
with col4:
    st.metric("Average Yield", str(round(filtered_df["Yield"].mean(), 2)) + "%")
with col5:
    st.metric("Average Investor Score", str(round(filtered_df["Investor_Score_(Out_Of_100)"].mean(), 1)))
